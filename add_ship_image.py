#!/usr/bin/env python3
# Script para adicionar geração de imagem no comando ship

with open('caosbot_railway.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Adicionar imports do Pillow no início
...
# Adicionar função de criar imagem de ship (antes do comando @bot.command(name='ship'))
ship_image_function = """
# ========================================
...
# Encontrar onde inserir (antes do @bot.command(name='ship'))
insert_position = content.find("@bot.command(name='ship')")
if insert_position != -1:
    content = content[:insert_position] + ship_image_function + content[insert_position:]

# Salvar
with open('caosbot_railway.py', 'w', encoding='utf-8') as f:
    f.write(content)

print("Funcao de gerar imagem de ship adicionada!")
