...
class MusicQueue:
    def __init__(self):
        ...
        self.bot_name = None  # Nome do bot que está tocando
...
# Função para verificar se Lavalink está acessível
async def wait_for_lavalink():
    """Aguarda o Lavalink estar disponível"""
    print("⏳ Verificando disponibilidade do Lavalink...")
    for attempt in range(1, 21):
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(LAVALINK_URL, timeout=aiohttp.ClientTimeout(total=3)) as resp:
                    if resp.status in [200, 404]:  # 404 é ok, significa que está rodando
                        print(f"✅ Lavalink acessível em {LAVALINK_URL}")
                        return True
        except Exception as e:
            print(f"Tentativa {attempt}/20: Lavalink não respondeu ({str(e)[:50]})")
            await asyncio.sleep(2)
    
    print("❌ Lavalink não respondeu após 20 tentativas")
    return False
...
class MusicControlPanel(View):
    def __init__(self, ctx, queue_obj):
        super().__init__(timeout=None)
        self.ctx = ctx
        self.queue_obj = queue_obj

    async def update(self):
        if not self.queue_obj.control_message:
            return
        embed = await self.create_embed()
        try:
            await self.queue_obj.control_message.edit(embed=embed, view=self)
        except:
            pass

    async def create_embed(self):
        if not self.queue_obj.current:
            return discord.Embed(title="🎵 Nenhuma música tocando", color=COLOR_ERROR)
        
        track = self.queue_obj.current
        embed = discord.Embed(
            title="🎵 Tocando Agora",
        ...
        embed.add_field(
            name="👤 Pedido por",
            value=f"<@{self.queue_obj.current_requester}>",
            inline=True
        )
        ...
        embed.add_field(
            name="🔊 Volume",
            value=f"{self.queue_obj.volume}%",
            inline=True
        )
        
        embed.add_field(
            name="ℹ️ Info",
            value=f"🔁 Loop: {self.queue_obj.loop_mode}\n📋 Na fila: {len(self.queue_obj.queue)}",
            inline=False
        )
    ...
    async def pause_resume(self, interaction: discord.Interaction, button: Button):
        ...
        if not player:
            return await interaction.followup.send(
                embed=discord.Embed(description="❌ Nenhuma música tocando.", color=COLOR_ERROR),
                ephemeral=True
            )
        ...
        if player.paused:
            ...
            await interaction.followup.send(
                embed=discord.Embed(description="▶️ Retomado.", color=COLOR_SUCCESS),
                ephemeral=True
            )
        else:
            await player.pause(True)
            await interaction.followup.send(
                embed=discord.Embed(description="⏸️ Pausado.", color=COLOR_SUCCESS),
                ephemeral=True
            )
    ...
    async def skip(self, interaction: discord.Interaction, button: Button):
        ...
        if not member.voice or member.voice.channel != player.channel:
            return await interaction.followup.send(
                embed=discord.Embed(description="❌ Você precisa estar na call!", color=COLOR_ERROR),
                ephemeral=True
            )
        
        queue_obj = get_queue(interaction.guild.id)
        queue_obj.skip_votes.add(member.id)
        
        members_in_voice = len([m for m in player.channel.members if not m.bot])
        votes_needed = math.ceil(members_in_voice * 0.6) if members_in_voice > 0 else 1
        current_votes = len(queue_obj.skip_votes)
        
        if current_votes >= votes_needed:
            await player.stop()
            queue_obj.skip_votes.clear()
            await interaction.followup.send(
                embed=discord.Embed(description=f"⏭️ Pulado! ({current_votes}/{votes_needed})", color=COLOR_SUCCESS)
            )
        else:
            await interaction.followup.send(
                embed=discord.Embed(description=f"🗳️ Voto registrado ({current_votes}/{votes_needed})", color=COLOR_SUCCESS),
                ephemeral=True
            )
    ...
    async def loop_button(self, interaction: discord.Interaction, button: Button):
        ...
        if queue_obj.loop_mode == 'off':
            queue_obj.loop_mode = 'song'
            msg = "🔂 Loop da música ativado."
        elif queue_obj.loop_mode == 'song':
            queue_obj.loop_mode = 'queue'
            msg = "🔁 Loop da fila ativado."
        else:
            queue_obj.loop_mode = 'off'
            msg = "➡️ Loop desativado."
        
        await interaction.followup.send(
            embed=discord.Embed(description=msg, color=COLOR_SUCCESS),
            ephemeral=True
        )
        await self.update()
    ...
    async def stop_button(self, interaction: discord.Interaction, button: Button):
        ...
        if queue_obj.current_requester:
            is_requester = interaction.user.id == queue_obj.current_requester
            is_mod = interaction.user.guild_permissions.manage_messages
            if not (is_requester or is_mod):
                return await interaction.followup.send(
                    embed=discord.Embed(description="❌ Apenas quem pediu ou mods!", color=COLOR_ERROR),
                    ephemeral=True
                )
        ...
        if player:
            ...
            for name, st in bot_pool.items():
                if st['guild_id'] == interaction.guild.id:
                    mark_bot_free(name)
    ...
    @discord.ui.button(label="📋 Fila", style=discord.ButtonStyle.secondary, row=2)
    async def queue_button(self, interaction: discord.Interaction, button: Button):
        await interaction.response.defer()
        queue_obj = get_queue(interaction.guild.id)
        
        if not queue_obj.queue and not queue_obj.current:
            return await interaction.followup.send(
                embed=discord.Embed(description="📋 Fila vazia!", color=COLOR_ERROR),
                ephemeral=True
            )
        
        embed = discord.Embed(title="📋 Fila de Músicas", color=COLOR_SUCCESS)
        ...
        if queue_obj.queue:
            text = ""
            for i, track in enumerate(list(queue_obj.queue)[:10], 1):
                text += f"`{i}.` {track.title}\n"
            embed.add_field(name="📝 Próximas", value=text, inline=False)
        
        await interaction.followup.send(embed=embed, ephemeral=True)

# ========================================
# SISTEMA SIMPLIFICADO - 1 BOT PARA TUDO
# ========================================
# O bot principal já está configurado no topo do arquivo
# Wavelink permite múltiplos players simultâneos no mesmo bot!
...
async def play(ctx, *, query: str):
    """.play <nome|link> ou .p <nome|link>"""
    if not ctx.author.voice:
        return await ctx.reply(
            embed=discord.Embed(description="❌ Você precisa estar em um canal de voz!", color=COLOR_ERROR)
        )
    
    queue_obj = get_queue(ctx.guild.id)
    
    msg = await ctx.reply(
        embed=discord.Embed(description="🔎 Buscando música...", color=COLOR_SUCCESS)
    )
    ...
    try:
        ...
        if not tracks:
            return await msg.edit(
                embed=discord.Embed(description="❌ Nenhum resultado encontrado.", color=COLOR_ERROR)
            )
        track = tracks[0]
    except Exception as e:
        return await msg.edit(
            embed=discord.Embed(description=f"❌ Erro ao buscar: {e}", color=COLOR_ERROR)
        )
    
    # Conectar ou pegar player existente
    ...
    if not player:
        try:
            ...
            return await msg.edit(
                embed=discord.Embed(description=f"❌ Erro ao conectar: {e}", color=COLOR_ERROR)
            )
    
    # Se já está tocando, adicionar na fila
    if player.playing:
    ...
    await player.set_volume(queue_obj.volume)
    print(f"🎵 Tocando: {track.title} no servidor {ctx.guild.name}")
    ...
    return await msg.edit(
        embed=discord.Embed(description=f"▶️ Tocando: **{track.title}**", color=COLOR_SUCCESS)
    )
...
async def skip_cmd(ctx):
    ...
    queue_obj = get_queue(ctx.guild.id)
    queue_obj.skip_votes.add(ctx.author.id)
...
async def stop_cmd(ctx):
    ...
    if queue_obj.current_requester:
        is_requester = ctx.author.id == queue_obj.current_requester
...
async def resume_cmd(ctx):
    ...
    if not player or not player.paused:
        return await ctx.reply(
            embed=discord.Embed(description="❌ A música não está pausada.", color=COLOR_ERROR)
        )
    await player.pause(False)
    return await ctx.reply(
        embed=discord.Embed(description="▶️ Retomado.", color=COLOR_SUCCESS)
...
async def queue_cmd(ctx):
    ...
    embed = discord.Embed(title="📋 Fila", color=COLOR_SUCCESS)
...
async def volume_cmd(ctx, value: int):
    if not 0 <= value <= 100:
        return await ctx.reply(
            embed=discord.Embed(description="❌ Volume deve estar entre 0 e 100.", color=COLOR_ERROR)
        )
    
    player: wavelink.Player = ctx.guild.voice_client
    queue_obj = get_queue(ctx.guild.id)
    queue_obj.volume = value
    
    if player:
        await player.set_volume(value)
    
    return await ctx.reply(
        embed=discord.Embed(description=f"🔊 Volume ajustado para {value}%", color=COLOR_SUCCESS)
    )
...
async def shuffle_cmd(ctx):
    ...
    if len(queue_obj.queue) < 2:
        return await ctx.reply(
            embed=discord.Embed(description="❌ Precisa ter pelo menos 2 músicas!", color=COLOR_ERROR)
        )
    
    q = list(queue_obj.queue)
    random.shuffle(q)
    queue_obj.queue = deque(q)
    return await ctx.reply(
        embed=discord.Embed(description="🔀 Fila embaralhada.", color=COLOR_SUCCESS)
    )
...
async def leave_cmd(ctx):
    ...
    await player.disconnect()
    return await ctx.reply(
        embed=discord.Embed(description="👋 Saí do canal.", color=COLOR_SUCCESS)
    )
